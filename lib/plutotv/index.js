	...
	const process = async (config) => {
		...
		const getRegion = async (region) => {
			...
			try {
				...
				fs.writeFileSync(`${outdir}/plutotv_smokehouse${region}.m3u8`, m3u8, 'utf-8');
				fs.writeFileSync(`${outdir}/plutotv_smokehouse${region}.xml`, xmltv, 'utf-8');
	...
	const process = async (config) => {
		...
		const getRegion = async (region) => {
			...
			try {
				...
				console.log("generating xmltv");
				const xmltv = await api.generateXMLTV(region, regionalize);
				fs.writeFileSync(`${outdir}/plutotv_ultimate${region}.m3u8`, m3u8, 'utf-8');
				fs.writeFileSync(`${outdir}/plutotv_ultimate${region}.xml`, xmltv, 'utf-8');
